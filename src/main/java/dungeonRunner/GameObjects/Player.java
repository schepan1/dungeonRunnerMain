...
public class Player {
	...
	public void update(int delta) throws transferException{
		...
		if (currentMap.isDoor(Player.x + 32, Player.y + 32 + dungeonRunnerEngine.fdelta)) {
			dungeonRunnerEngine.mapSet.get(currentMap.ID).blocked = null; // Alte Map berechnung wird zur�ckgesetzt.
			currentMap.transfer.play(0.5f, 0.4f);						  // Transfer Sound
			schritte = 0;
			currentMap = dungeonRunnerEngine.mapSet.get(currentMap.ID+1); //Hier wird n�chste Map initalisiert.
			currentMap.init();
